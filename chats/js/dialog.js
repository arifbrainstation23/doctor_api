    ...
    QB.chat.dialog.create(params, function (err, createdDialog) {
        if (err) {
            ...
            var occupants = createdDialog.occupants_ids,
                   ...
                   chatEL2 = localStorage.getItem('chatEL'),
                   //chatEL2 =  JSON.parse(userX2),

                message_body = (chatEL2 || chatEL2) + ' created new dialog with: ';
          
                console.log(chatEL2);
            _.each(occupants, function (occupantId) {
                 var chatEL = localStorage.getItem('chatEL');
               //  var chatEL =  JSON.parse(userX);
           
               // message_body += (userModule._cache[occupantId].name || userModule._cache[occupantId].login) + " ";
               message_body += (chatEL ||chatEL) + " ";
